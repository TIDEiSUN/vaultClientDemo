...
if (!online) {
  ...
  mockBlob.get('/v1/authinfo?username=' + exampleData.username)
    .reply(200, JSON.stringify(authInfoRes.body), {
      'Content-Type': 'application/json'
    });   

  mockBlob.get('/v1/authinfo?username=' + exampleData.new_username)
    .reply(200, JSON.stringify(authInfoNewUsernameRes.body), {
      'Content-Type': 'application/json'
    });

  mockBlob.get('/v1/authinfo?username=' + exampleData.blob.data.account_id)
  .reply(200, JSON.stringify(authInfoNewUsernameRes.body), {
    'Content-Type': 'application/json'
  });    
...
describe('Ripple Txt', function () {
  it('should get the content of a ripple.txt file from a given domain', function(done) {
    RippleTxt.get(exampleData.domain).then((resp) => {
  ...
  it('should get currencies from a ripple.txt file for a given domain', function(done) {
    RippleTxt.getCurrencies(exampleData.domain).then((currencies) => {
  ...
  it('should get the domain from a given url', function() {
    const domain = RippleTxt.extractDomain("http://www.example.com");
...
describe('VaultClient', function () {
  ...
  describe('#exists', function() {
    it('should determine if a username exists on the domain', function(done) {
      this.timeout(10000);
      client.exists(exampleData.username).then((resp) => {
  ...
  describe('#addressExists', function() {
    it('should determine if an address exists on the domain', function(done) {
      this.timeout(10000);
      client.addressExists(exampleData.blob.data.account_id).then((resp) => {
        assert.strictEqual(typeof resp, 'boolean');
        done();
      }).catch((err) => {
        assert.ifError(err);
  ...
  describe('#login', function() {
    it('with username and password should retrive the blob, crypt key, and id', function(done) {
      this.timeout(10000);
      client.login(exampleData.username, exampleData.password, exampleData.device_id).then((resp) => {
        if (online) {
          assert.strictEqual(typeof resp, 'object');
  ...
  describe('#relogin', function() {
    it('should retrieve the decrypted blob with blob vault url, id, and crypt key', function(done) {
      this.timeout(10000);
      client.relogin(exampleData.blob.url, exampleData.id, exampleData.crypt, exampleData.device_id).then((resp) => {
        assert.strictEqual(typeof resp, 'object');
        checkBlob(resp.blob);
        done();
      }).catch((err) => {
  ...
  describe('#unlock', function() {
    it('should access the wallet secret using encryption secret, username and password', function (done) {
      this.timeout(10000);
      client.unlock(exampleData.username, exampleData.password, blobRes.encrypted_secret).then((resp) => {
        if (online) {
  ...
  describe('#loginAndUnlock', function () {
    it('should get the decrypted blob and decrypted secret given name and password', function (done) {
      this.timeout(10000);
      client.loginAndUnlock(exampleData.username, exampleData.password, exampleData.device_id).then((resp) => {
        if (online) {
          assert.strictEqual(typeof resp, 'object');
  ...
  describe('#register', function () {
    it('should create a new blob', function (done) {
      ...
      client.register(options).then((resp) => {
        assert(false);
        done();
      }).catch((err) => {
  ...
  describe('#deleteBlob', function () {
    it('should remove an existing blob', function (done) {
      ...
      client.deleteBlob(options).then((resp) => {
        assert.strictEqual(typeof resp, 'object');
...
describe('Blob', function () {
  ...
  describe('#rename', function () {
    it('should change the username of a blob', function (done) {
      ...
      client.rename(options).then((resp) => {
        assert.strictEqual(typeof resp, 'object');
        assert.strictEqual(typeof resp.result,  'string');
  ...
  describe('#changePassword', function () {
    it('should change the password and keys of a blob', function (done) {
      ...
      client.changePassword(options).then((resp) => {
        assert.strictEqual(typeof resp, 'object');
        assert.strictEqual(typeof resp.result,  'string');
        assert.strictEqual(typeof resp.message, 'string');    
        done();
  ...
  describe('#recoverBlob', function () {
    it('should recover the blob given a username and secret', function (done) {
      ...
      client.recoverBlob(options).then((blob) => {
  ...
  describe('#verifyEmail', function () {
    it('should verify an email given a username and token', function (done) {
      ...
      client.verify(exampleData.username, exampleData.email_token).then((resp) => {
        assert(false);
        done();
      }).catch((err) => {
  ...
  describe('#resendVerifcationEmail', function () {
    it('should resend a verification given options', function (done) {
      ...
      client.resendEmail(options).then((resp) => {
        assert.strictEqual(typeof resp, 'object');
        assert.strictEqual(typeof resp.result,  'string');
        done();
      }).catch((err) => {
  ...
  it('#unset', function(done) {
    this.timeout(10000)
    blob.unset('/testObject').then((resp) => {
      assert.strictEqual(resp.result, 'success');
      done();
    }).catch((err) => {
      assert.ifError(err);
  ...
  it('#filter', function(done) {
    ...
    blob.filter('/testArray', 'name', 'bob', 'extend', '', {description:'Alice'}).then((resp) => {
      assert.strictEqual(resp.result, 'success');
      done();
    }).catch((err) => {
      assert.ifError(err);
  ...
  it('#consolidate', function(done) {
    this.timeout(10000)
    blob.unset('/testArray').then((resp) => {
      assert.strictEqual(resp.result, 'success');
      return blob.consolidate();
    }).then((resp) => {
        assert.strictEqual(resp.result, 'success');
        done();
    }).catch((err) => {
  ...
  describe('identity', function() {
    it('#identity_set', function (done) {
      ...
      blob.identity.set('address', exampleData.unlock, {city:"San Francisco", region:"CA"}).then((resp) => {
        assert.strictEqual(resp.result, 'success');
        done();          
      }).catch((err) => {
        assert.ifError(err);
    ...
    it('#identity_get', function () {
      const property = blob.identity.get('address', exampleData.unlock);
    ...
    it('#identity_getAll', function () {
      const obj = blob.identity.getAll(exampleData.unlock);  
      assert.strictEqual(typeof obj, 'object');       
    }); 

    it('#identity_getFullAddress', function () {
      const address = blob.identity.getFullAddress(exampleData.unlock);  
    ...
    it('#identity_unset', function (done) {
      ...
      blob.identity.unset('name', exampleData.unlock).then((resp) => {
        assert.strictEqual(resp.result, 'success');
        done();          
      }).catch((err) => {
        assert.ifError(err);
  ...
  describe('identityVault', function() {
    it('#identity - Get Attestation', function (done) {
      ...
      client.getAttestation(options).then((resp) => {
        assert.strictEqual(resp.result, 'success');
    ...
    it('#identity - Update Attestation', function (done) {
      ...
      client.updateAttestation(options).then((resp) => {
        assert.strictEqual(resp.result, 'success');
        assert.strictEqual(typeof resp.attestation, 'string'); 
        assert.strictEqual(typeof resp.blinded, 'string'); 
        assert.deepEqual(resp.decoded, {"header":{"z":"z"},"payload":{"z":"z"},"signature":"sig"})
    ...
    it('#identity - Get Attestation Summary', function (done) {
      ...
      client.getAttestationSummary(options).then((resp) => {
        assert.strictEqual(resp.result, 'success');
        assert.strictEqual(typeof resp.attestation, 'string'); 
  ...
  if (!online) {
    ...
    it('#2FA_set2FA', function (done) {
      blob.set2FA({masterkey:exampleData.masterkey}).then((resp) =>{
        assert.strictEqual(typeof resp, 'object');
        assert.strictEqual(typeof resp.result,  'string');
        done();    
      }).catch((err) => {
    ...
    it('#2FA_get2FA', function (done) {
      ...
      it('#2FA_requestToken', function (done) {
        client.requestToken(exampleData.blob.url, exampleData.blob.id).then((resp) =>{
          assert.strictEqual(typeof resp, 'object');
          assert.strictEqual(typeof resp.result,  'string');
          done();
        }).catch((err) => {
      ...
      it('#2FA_verifyToken', function (done) {
        ...
        client.verifyToken(options).then((resp) => {
          assert.strictEqual(typeof resp, 'object');
          assert.strictEqual(typeof resp.result,  'string');
          done();          
        }).catch((err) => {
